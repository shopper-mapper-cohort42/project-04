...
export default function Results({ apiKey }) {
    ...
    return (
        <div>
            <Link to={`/location/${currentLocation.longitude},${currentLocation.latitude}`} className="findLocation">
                BACK
            </Link>
            ...
            */}
            <form onSubmit={handleSubmitSearchRadiusChange}>
                ...
                <input type="text" id="searchRadiusInput" value={searchRadiusInput} onChange={handleSearchRadiusInputChange} />
                        ...
                        <li key={result.id}>
                            {
                                ...
                                indicesToHighlight.indexOf(resultIndex) >= 0 ? <h2>THIS IS THE HIGHLIGHTED RENDERING</h2> : null // null is the NON-HIGHLIGHTED RESULT
                            }
                            <h3>{result.name}</h3>
                            <p>{result.displayString}</p>
                            <p>{lonLatDistance(currentLocation.longitude, currentLocation.latitude, resultLocation.longitude, resultLocation.latitude).toFixed(2)} km away</p>
                        </li>
                    );
                })}
            </ol>
