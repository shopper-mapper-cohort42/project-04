...
import { faBicycle, faCar, faClock, faRoad, faWalking, faAngleLeft, faAngleRight } from '@fortawesome/free-solid-svg-icons';
...
export default function Directions({ apiKey, mapState, destination, directionsLayer, setDirectionsLayer, directionsLayerDefined, setDirectionsLayerDefined }) {
    ...
    }, [apiKey, destinationCoords, routeTypeInput]);
    // const routeOptions =  {
    //     start: `${currentLocation.latitude},${currentLocation.longitude}`,
    //     end: destinationCoords,
    //     options: {
    ...
    }, [apiKey, directionsLayer, directionsLayerDefined, mapState, routeOptions, setDirectionsLayer, setDirectionsLayerDefined]);

    useEffect(() => {
        mapthing();
    }, [mapthing]); //Update the directions when mounted and whenever the destination changes

    const mapthing2 = useCallback(() => {
        if (directionsLayerDefined) {
        ...
        <section className="directionsSection">
            <div className="wrapper">
                <div className="directionBackButton">
                    <Link to={`/location/${currentLocation.longitude},${currentLocation.latitude}/${searchItem}`} className="backButton returnLinks returnToMain resultBack">
                ...
                <div className={hideDirections ? 'directionsDiv active' : 'directionsDiv'}>
                    ...
                    <button
                        ...
                        onClick={() => {
                            hideDirections ? setHideDirections(false) : setHideDirections(true);
                        }}
                    >
                        <FontAwesomeIcon icon={hideDirections ? faAngleLeft : faAngleRight} />
                    </button>
                    <div className={hideDirections ? 'directionsTopContainer active' : 'directionsTopContainer'}>
                        <div className="destinationInfoHeadingContainer">
                            <h3>Directions to {destination.name}</h3>

                            <h4>{destination.displayString ? destination.displayString.split(`${destination.name},`) : ''}</h4>
                        </div>
                            ...
                            <ul>
                                ...
                                <li>
                                    ...
                                    <label htmlFor="routeTypeInputPedestrian">
                                        <FontAwesomeIcon className="directionIcon" icon={faWalking} />
                                        <> Walking</>
                                    </label>
                                </li>
                                <li>
                                    ...
                                    <label htmlFor="routeTypeInputBicycle">
                                        <FontAwesomeIcon className="directionIcon" icon={faBicycle} />
                                        <> Bicycle</>
                                    </label>
                                </li>
                            </ul>
